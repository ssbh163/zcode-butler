...
>
> 当前状态:**M1-M4 全部交付,悬浮窗至 v0.4.6(1/3 锚定 + 容纳不下自动隐藏;Key 环已用额度三色)**(交互细项待真机人工验收,清单见开发日志 M2 条)。
...
- 已知限制:C# 宿主内部 SetWindowTextW 不生效之谜未解(标题乱码,截图工具已改按窗口类名找窗,无实害);**拖动手感/点击穿透(月牙区应可点到 ZCode)/键盘输入待真机人工验收**(跟随移动已验证 ✓ v0.4.5 用户实机确认零残影;最小化/恢复已验证 ✓ v0.4.1;1/3 锚定与容纳显隐已验证 ✓ v0.4.6 程序化缩窗实测);ZCode 窗高不足(当前 < ≈1370 物理px = 1.5×侧栏可见高)时侧栏整体隐藏、恢复自动重现(v0.4.6);窗高介于 1370–1575 时窗口透明尾区越出 ZCode 底缘,底部环悬停弹窗可能渲染到窗外桌面区(纯渲染,不在命中掩码内);运行需系统 WebView2 Runtime 且**版本须与 vendored SDK 同代**(4191 配 152;升级 DLL 时按构建号配对);原生设置卡(归档入口/Key 增删/设置)与资讯面板未在 HTML 内重建;高峰判定用页面本机时间(status.mjs 口径是服务端北京时间,无实害)
